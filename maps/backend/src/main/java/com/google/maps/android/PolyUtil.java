...
public class PolyUtil {
  ...
  /**
   * Computes whether the vertical segment (lat3, lng3) to South Pole intersects the segment (lat1,
   * lng1) to (lat2, lng2). Longitudes are offset by -lng1; the implicit lng1 becomes 0.
   */
  ...
  /**
   * Computes whether the given point lies inside the specified polygon. The polygon is always
   * considered closed, regardless of whether the last point equals the first or not. Inside is
   * defined as not containing the South Pole -- the South Pole is always outside. The polygon is
   * formed of great circle segments if geodesic is true, and of rhumb (loxodromic) segments
   * otherwise.
   */
   ...
   * tolerance in meters. The polygon edge is composed of great circle segments if geodesic is true,
   * and of Rhumb segments otherwise. The polygon edge is implicitly closed -- the closing segment
  ...
  /**
   * Same as {@link #isLocationOnEdge(LatLng, List, boolean, double)} with a default tolerance of
   * 0.1 meters.
   */
  public static boolean isLocationOnEdge(LatLng point, List<LatLng> polygon, boolean geodesic) {
  ...
  /**
   * Computes whether the given point lies on or near a polyline, within a specified tolerance in
   * meters. The polyline is composed of great circle segments if geodesic is true, and of Rhumb
   * segments otherwise. The polyline is not closed -- the closing segment between the first point
  ...
  /**
   * Returns sin(initial bearing from (lat1,lng1) to (lat3,lng3) minus initial bearing from (lat1,
   * lng1) to (lat2,lng2)).
   */
   ...
   *
   * When the providing a polygon as input, the first and last point of the list MUST have the same
   * latitude and longitude (i.e., the polygon must be closed). If the input polygon is not closed,
   * the resulting polygon may not be fully simplified.
   *
